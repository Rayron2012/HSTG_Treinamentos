...
for estado in niveis_co2:
    ...
    total_niveis = sum(niveis_co2[estado]) #pode ser usado operações matematicas para somar valores dentro dos dicionarios
    media = total_niveis / qtd_sensores

    if media > 450:
        print("{} esta com níveis altissimos de CO2 ({}), chamar equipe especializada para o local!".format(estado, media))
'''
...
Para resolver isso, a gente fez a integração e fizemos uma "requisição" de todos os vídeos para a Vimeo. Essa 
requisição dá como resposta para o nosso código isso:
'''
...
No nosso caso, tínhamos uma lista com os mais de 2.000 vídeos (sendo cada vídeo um dicionário exatamente como esse aí 
em cima), e por isso fizemos exatamente esse mesmo procedimento que você vai construir aqui abaixo, só que dentro de um
"for" para percorrer todos os vídeos. Não podemos disponibilizar a lista inteira por questões de segurança e proteção 
de dados mesmo, mas o dicionário acima já vale o exemplo.'''
